document.addEventListener('DOMContentLoaded', () => {
    ...
    const fetchComments = async () => {
        try {
            const response = await fetch('http://localhost:3001/fcomments'); // Assuming you have a route to fetch comments
    ...
    commentForm.addEventListener('submit', async (event) => {
        ...
        try {
            const response = await fetch('http://localhost:3001/fcomment', {
                method: 'POST',
                headers: {
